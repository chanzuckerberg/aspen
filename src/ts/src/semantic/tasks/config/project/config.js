...
module.exports = {
  ...
  addDerivedValues: function (config) {
    ...
    const components =
      Array.isArray(config.components) && config.components.length >= 1
        ? config.components
        : defaults.components;
    const individuals =
      Array.isArray(config.individuals) && config.individuals.length >= 1
        ? config.individuals
        : [];
    const componentsExceptIndividuals = components.filter(
      (component) => !individuals.includes(component)
    );
    ...
    config.globs.individuals =
      individuals.length >= 1 ? "{" + individuals.join(",") + "}" : undefined;
